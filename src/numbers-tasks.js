...
function getCircleCircumference(radius) {
  const RadiusNew = radius;
  const Pi = Math.PI;
  const Line = Pi * RadiusNew * 2;
  return Line;
}
...
function roundToPowerOfTen(num, pow) {
  const num1 = Math.round(num / 10 ** pow) * 10 ** pow;
  return num1;
}
...
function isInteger(number) {
  const num = Number.isInteger(number);
  if (num) {
    return true;
  }
  return false;
...
function isSafeInteger(number) {
  const num = Number.isSafeInteger(number);
  if (num) {
    return true;
  }
  return false;
